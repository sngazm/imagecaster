...
しかも付くときは付きすぎる（#286 の正解と比べて読点が 2 倍以上、「〜けど」「〜て」で
終わる文が読点で次に繋がり一文が 100 字を超える）。`punctuate.py` が全文を 40 行ずつ
claude に渡して**整える**。単語の時刻から、話し手が息を置いた位置（0.4 秒以上の間）を
「▁」、次の行まで 1 秒以上空くところを「▁▁」として本文に埋めて見せ、次の決まりで
付け直させる: 読点は少なめに（意味の切れ目だけ）、「〜けど」「〜て」で間があり内容が
移るなら「。」、言いさしは「…」、相槌は「。」で閉じる、一文は 60 字目安。句読点以外が
変わった答えは捨てる（貼り直しが狂うと本文を静かに壊す）。3 並列で呼ぶ。

以前は句読点の無い行に足すだけだった（さらに前は「窓の 15% 未満」の区間だけで、hotwords
以降の交互に出る出力に一度も反応せず #281 の 30〜45 分が読点なしのまま公開された）。
句読点の付き方は CER にも話者一致にも映らないので、`scripts/transcript-eval/punct_diff.py`
（正解と突き合わせて「正解は。／こちらは、」のような食い違いを数える）と
`punct_check.py`（時間帯ごとの割合）で確かめる。
