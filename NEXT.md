...
**設定 → 文字起こし** に「校正からの提案」が並んでいる。Claude が見つけた誤りの
うち、番組全体に効きそうなものだ。1 件ずつ「辞書に入れる」か「見送る」かを選ぶ。

**辞書には自動で入らない。** 一度自動登録にしていたが、`メール → mail`
`テスト → 鉄塔` `修正 → 習性` のような規則が入って公開中の文章を壊した。
歯止めを足すたびに別の抜け道が見つかったので、承認を挟む形にした。

現在 154 件溜まっている。大半は正しい（`アサナ → Asana` `TMAX → tmux`）が、
`確信 → 核心` `行動 → 公道` のように、それ自体が正しい語を壊すものも混じっている。
...
これが 0 件を返したときに終わりとする。**ただし 0 件は出発点で、終わりではない。**
監査は既知の形しか見ないので、実際の文面も数十行読む。

```bash
node /tmp/passage.mjs 285 40 20   # ep, 開始行, 行数
```

テストが通ってデータが正しく保存されていても、公開されたページが読み物として
壊れていることがある。実際に何度もあった。

| 見つけたもの | 気づいた方法 |
|---|---|
| ハルシネーション「ヤンヤン」883件 | 指摘を受けた |
| `メール → mail` の自動登録 | サイトを読んだ |
| 話者境界が文の途中に落ちる | サイトを読んだ |
| 拾い直しがハルシネーションを作る | 出力を読んだ |
| 「GoogleWorkspace」「え?で、」 | サイトを読んだ |

見つけたものは監査の項目に足してある。次からは自動で見つかる。
