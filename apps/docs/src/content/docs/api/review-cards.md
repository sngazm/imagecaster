...
**カードを出すのは、トラック別の音源から起こした回だけです。** トラックが無い回は元の品質が
低く、1 枚ずつ聞いて決める量に収まらないので、校正はしてもカードは送りません。判定は
transcriber 側で、文字起こしのときは話者判定器を用意できたか、公開済みの回に見直しだけを
かけ直すときは meta の `speakerTrackFiles` / `levelsUrl` / `tracksUploadedAt` のどれかがあるか
で決めます（zip は処理後に消すので `tracksUploadedAt` だけでは足りません）。
