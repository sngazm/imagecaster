...
| `pp_runner.ts raw.json index.json meta.json out.json [global\|full]` | Worker の整形をローカルで同じ引数で呼ぶ。公開データと一致することを確認済み。esbuild で bundle して node で実行 |
| `pp_stages.ts raw.json index.json meta.json prefix` | 整形を 1 段ずつ書き出す。`SKIP_REPAIR=1` で repairSpeakerBoundaries を飛ばす |
